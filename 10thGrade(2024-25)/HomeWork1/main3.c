...
int main(void) {
    ...
    // int arr[] = {};
    // int arr[] = {100000, 50001, 100000, 2000000, 3003, 4123};
...
int getNumber(int num, int pos) { //dqno na lqwo
    for (int i = 1; i < pos; i++) {
        num /= 10;
    }
    return num % 10; 
...
// Като започваме с най-малките цифри(от LSB към MSB) постепенно се изгражда правилния ред. Така числата с по-малък брой цифри се подреждат правилно още в началото. А при MSB към LSB, те се подреждат един вид от зад напред. Отделно при сортиране на числата с по-малки цифири те постепенно се събират подредени в buckets[0].
